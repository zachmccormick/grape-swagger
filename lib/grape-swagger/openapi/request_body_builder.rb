    ...
    class RequestBodyBuilder
      ...
      class << self
        ...
        def build(params, method, consumes, version)
          ...
          # Determine content types - use consumes if available, fallback to default
          content_types = resolve_content_types(consumes)
          return nil if content_types.empty?

          # Build the requestBody object
          {
            required: determine_required(body_params),
            content: build_content(body_params, content_types, version),
        ...
        #
        # @param body_params [Array<Hash>] Body parameters
        # @param content_types [Array<String>] Media types
        # @param version [GrapeSwagger::OpenAPI::Version] The OpenAPI version
        # @return [Hash] Content object with media type keys
        def build_content(body_params, content_types, version)
          content_types.each_with_object({}) do |media_type, content|
        ...
        def build_merged_schema(body_params, version)
          ...
          body_params.each do |param|
            if param[:properties]
              ...
              prop_schema = build_property_schema(param)
        ...
        # Build a property schema from a parameter, including all Phase 6 enhancements
        #
        # @param param [Hash] Parameter definition
