...
describe GrapeSwagger::OpenAPI::ParameterSchemaWrapper do
  ...
  describe '.wrap' do
    # Story 9.1: Parameter Schema Wrapping - Tests for wrapping type/format/enum in schema
    context 'Story 9.1: Parameter Schema Wrapping' do
      ...
      context 'extended schema fields (Phase 6)' do
        it 'wraps readOnly in schema object' do
          parameter = { name: 'id', in: 'query', type: 'integer', readOnly: true }

          result = described_class.wrap(parameter, version_3_1_0)

          expect(result[:schema][:readOnly]).to be true
          expect(result).not_to have_key(:readOnly)
        end

        it 'wraps writeOnly in schema object' do
          parameter = { name: 'password', in: 'query', type: 'string', writeOnly: true }

          result = described_class.wrap(parameter, version_3_1_0)

          expect(result[:schema][:writeOnly]).to be true
          expect(result).not_to have_key(:writeOnly)
        end

        it 'wraps minProperties and maxProperties in schema object' do
          parameter = { name: 'filter', in: 'query', type: 'object', minProperties: 1, maxProperties: 5 }

          result = described_class.wrap(parameter, version_3_1_0)

          expect(result[:schema][:minProperties]).to eq(1)
          expect(result[:schema][:maxProperties]).to eq(5)
          expect(result).not_to have_key(:minProperties)
          expect(result).not_to have_key(:maxProperties)
        end

        it 'wraps title in schema object' do
          parameter = { name: 'filter', in: 'query', type: 'string', title: 'Filter Parameter' }

          result = described_class.wrap(parameter, version_3_1_0)

          expect(result[:schema][:title]).to eq('Filter Parameter')
          expect(result).not_to have_key(:title)
        end

        it 'wraps not in schema object' do
          parameter = { name: 'status', in: 'query', type: 'string', not: { enum: ['deleted'] } }

          result = described_class.wrap(parameter, version_3_1_0)

          expect(result[:schema][:not]).to eq({ enum: ['deleted'] })
          expect(result).not_to have_key(:not)
        end

        it 'wraps externalDocs in schema object' do
          parameter = {
            name: 'format',
            in: 'query',
            type: 'string',
          ...
          expect(result[:schema][:externalDocs]).to eq({ url: 'https://example.com/docs' })
          expect(result).not_to have_key(:externalDocs)
        end
      end
    end

    # Story 9.2: Cookie Parameters - Tests for cookie parameter support
    context 'Story 9.2: Cookie Parameters' do
    ...
    # Story 9.3: Parameter Serialization - Tests for serialization options
    context 'Story 9.3: Parameter Serialization' do
    ...
    # Story 9.4: Phase 6 enhancements
    context 'Story 9.4: Phase 6 enhancements' do
      context '$ref passthrough' do
        it 'returns $ref parameters unchanged with symbol key' do
        ...
        it 'returns $ref parameters unchanged with string key' do
          parameter = { '$ref' => '#/components/parameters/PageSize' }
      ...
      context 'content/schema mutual exclusivity' do
        it 'skips schema wrapping when content is present' do
          parameter = {
            name: 'filter',
            in: 'query',
            content: {
        ...
        it 'does not add serialization options when content is present' do
          parameter = {
            name: 'filter',
            in: 'query',
            content: {
      ...
      context 'x-example to example conversion' do
        it 'converts x-example to example for OpenAPI 3.1.0 with symbol key' do
          parameter = { name: 'status', in: 'query', type: 'string', 'x-example': 'active' }

          result = described_class.wrap(parameter, version_3_1_0)

          expect(result[:example]).to eq('active')
          expect(result).not_to have_key(:'x-example')
        end

        it 'converts x-example to example for OpenAPI 3.1.0 with string key' do
          parameter = { name: 'status', in: 'query', type: 'string', 'x-example' => 'active' }
        ...
        it 'does not convert x-example for Swagger 2.0' do
          parameter = { name: 'status', in: 'query', type: 'string', 'x-example': 'active' }

          result = described_class.wrap(parameter, version_2_0)

          expect(result).not_to have_key(:example)
          expect(result).to have_key(:'x-example')
        end
      end
    end
    ...
    context 'edge cases' do
      ...
      it 'handles deprecated parameters' do
        parameter = { name: 'old_filter', in: 'query', type: 'string', deprecated: true }
      ...
      it 'handles example at parameter level' do
        parameter = { name: 'status', in: 'query', type: 'string', example: 'active' }
      ...
      it 'handles examples at parameter level' do
        parameter = {
          name: 'status',
          in: 'query',
          type: 'string',
          examples: { active: { value: 'active' }, inactive: { value: 'inactive' } }
        }

        result = described_class.wrap(parameter, version_3_1_0)

        expect(result[:examples]).to eq({ active: { value: 'active' }, inactive: { value: 'inactive' } })
        expect(result[:schema][:type]).to eq('string')
      end
    end
  end
