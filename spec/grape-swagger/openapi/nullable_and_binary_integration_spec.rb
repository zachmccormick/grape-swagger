...
describe 'Nullable and Binary Handling Integration' do
  ...
  describe 'DocMethods transform_nullable_types!' do
    it 'transforms nullable types in a nested hash structure' do
      output = {
        paths: {
          '/users' => {
            get: {
      ...
      GrapeSwagger::DocMethods.transform_nullable_types!(output)

      props = output[:paths]['/users'][:get][:responses][200][:schema][:properties]
      expect(props[:name][:type]).to eq(['string', 'null'])
      expect(props[:name]).not_to have_key(:nullable)
      expect(props[:age][:type]).to eq(['integer', 'null'])
      expect(props[:age]).not_to have_key(:nullable)
    end

    it 'handles nullable: false by leaving it unchanged' do
      output = {
      ...
      expect(output[:schema][:type]).to eq('string')
      expect(output[:schema][:nullable]).to eq(false)
    end
  end

  describe 'DocMethods transform_binary_formats!' do
    it 'transforms binary formats in a nested hash structure' do
      output = {
        paths: {
          '/upload' => {
            post: {
              parameters: [
                ...
                { name: 'data', type: 'string', format: 'byte' }
              ]
            }
          }
        }
      ...
      GrapeSwagger::DocMethods.transform_binary_formats!(output)

      params = output[:paths]['/upload'][:post][:parameters]
      expect(params[0][:contentEncoding]).to eq('base64')
      expect(params[0][:contentMediaType]).to eq('application/octet-stream')
      expect(params[0]).not_to have_key(:format)

      expect(params[1][:contentEncoding]).to eq('base64')
      expect(params[1]).not_to have_key(:contentMediaType)
      expect(params[1]).not_to have_key(:format)
    end

    it 'preserves custom contentMediaType' do
      output = {
        schema: { type: 'string', format: 'binary', contentMediaType: 'image/jpeg' }
      }

      GrapeSwagger::DocMethods.transform_binary_formats!(output)

      expect(output[:schema][:contentEncoding]).to eq('base64')
      expect(output[:schema][:contentMediaType]).to eq('image/jpeg')
      expect(output[:schema]).not_to have_key(:format)
    end

    it 'does not transform non-binary formats' do
      output = {
      ...
      expect(output[:schema][:format]).to eq('email')
      expect(output[:schema]).not_to have_key(:contentEncoding)
    end
  end
end
