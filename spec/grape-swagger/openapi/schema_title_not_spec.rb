...
describe 'Schema title and not Properties (OpenAPI 3.1.0)' do
  ...
  describe 'Schema title property' do
    describe 'parameter-level title' do
      def app
        Class.new(Grape::API) do
          ...
          params do
            ...
            optional :role, type: String, documentation: { title: 'User Role', desc: 'The assigned role' }
          end
      ...
      it 'includes title in path parameter schema' do
        ...
        # In OpenAPI 3.1.0, title is a Schema Object property
      ...
      it 'includes both title and description' do
        params = subject['paths']['/users/{user_id}']['get']['parameters']
        role_param = params.find { |p| p['name'] == 'role' }

        # In OpenAPI 3.1.0, title is inside schema, description stays at parameter level
        expect(role_param['schema']['title']).to eq('User Role')
        expect(role_param['description']).to eq('The assigned role')
      end
    end
  ...
  describe 'Schema not property' do
    ...
    describe 'not with const constraint' do
      def app
        Class.new(Grape::API) do
          format :json

          desc 'Parameter that must not equal specific value'
          params do
            requires :status, type: String, documentation: {
              not: { const: 'deleted' },
              desc: 'Status cannot be deleted'
            }
          end
          patch '/items/:id' do
      ...
      it 'includes not const constraint' do
        request_body = subject['paths']['/items/{id}']['patch']['requestBody']
        schema_ref = request_body['content']['application/json']['schema']['$ref']
        ...
        expect(status_schema['not']['const']).to eq('deleted')
      end
    end
