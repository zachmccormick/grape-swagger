...
describe 'Parameter content field (OpenAPI 3.1.0)' do
  ...
  describe 'query parameter with content' do
    ...
    it 'includes content in parameter instead of schema' do
    ...
    it 'does not include style/explode when content is present' do
      params = subject['paths']['/search']['get']['parameters']
      filter_param = params.find { |p| p['name'] == 'filter' }

      # style and explode are for schema-based parameters only
      expect(filter_param).not_to have_key('style')
      expect(filter_param).not_to have_key('explode')
    end
  end

  # ============================================
  # Header parameter with content
  # ============================================
  describe 'header parameter with content' do
    def app
      Class.new(Grape::API) do
        format :json

        desc 'Get with JSON header'
        params do
          optional :x_filter, type: String, documentation: {
            desc: 'JSON filter in header',
            param_type: 'header',
            content: {
              'application/json' => {
                schema: {
    ...
    it 'includes content in header parameter' do
      params = subject['paths']['/data']['get']['parameters']
      header_param = params.find { |p| p['in'] == 'header' }

      expect(header_param['content']).to be_a(Hash)
      expect(header_param['content']['application/json']['schema']['type']).to eq('object')
    end
  end
  ...
  # ============================================
  # Swagger 2.0 (no content support)
  # ============================================
